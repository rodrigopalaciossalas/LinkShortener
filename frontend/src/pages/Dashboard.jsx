                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md">
                            ...
                            <div className="text-3xl font-bold text-white">
                                {links.reduce((acc, link) => acc + link.click_count, 0)}
                            </div>
                         ...
                         <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-md">
                            <div className="text-gray-400 text-sm font-medium mb-1">Avg. Clicks / Link</div>
                            <div className="text-3xl font-bold text-white">
                                {links.length > 0 ? (links.reduce((acc, link) => acc + link.click_count, 0) / links.length).toFixed(1) : '0.0'}
                            </div>
                        ...
                        {loading ? (
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm text-left text-gray-400">
                                    <thead className="text-xs text-gray-400 uppercase bg-slate-700/50">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3 text-center">Created</th>
                                            ...
                                            <tr key={link.id} className="bg-slate-800 border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                                                <td className="px-6 py-4 font-medium text-white">
                                                    <div className="flex items-center">
                                                        ...
                                                        <div>
                                                            <div className="font-bold text-violet-300">{link.short_code}</div>
                                                            <div className="text-xs text-gray-500">{link.title || 'No Title'}</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <a href={`http://localhost:8000/${link.short_code}`} target="_blank" rel="noopener noreferrer" className="font-medium text-green-400 hover:underline mr-3">Test</a>
                             ...
                             <div className="p-12 text-center">
                                ...
                                <p className="text-gray-400 mb-6">Get started by creating your first short link from the button above.</p>
                                <Button variant="primary" className="w-auto" onClick={() => setIsCreateModalOpen(true)}>Create Link</Button>
                            </div>
                        )}
                    </div>
                </div>
